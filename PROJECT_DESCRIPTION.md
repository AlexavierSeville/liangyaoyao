...
- 分支 `main`，远程 `https://github.com/AlexavierSeville/liangyaoyao.git`；精确提交以 `git log -1` 为准。
- 肚子享受抚摸、左右翅膀轻抚握手与不耐烦均已完成正式接入。用户已批准统一身材、短翅膀及颜色后的版本。
- 翅膀握手每侧 16 帧、一轮 4.25 秒；不耐烦每侧 12 帧、2.04 秒。身体长按从按下计时 5–10 秒，400 ms 后开始循环。松手停止循环，但已触发的反应播完。
- 当前透明画布 368×228；身体命中区仍为 220×228；企鹅名义高度 198、脚底基线 215。翅膀正式 @2x 帧为 736×456，肚子保留 640×456；不要用整张画布尺寸推断人物大小。
- 播放器支持 `frameDurationsMs`。`FlipperHold` 优先级 50，`FlipperReact` 优先级 60，拖动优先级 100。
- 正式资源位于 `public/assets/animations/`。`output/`、回滚备份和录像拆帧保留在本机，不上传，也不是运行时依赖。
- 40 项交互测试、逐帧时序测试、实际浏览器前端演示、Windows 构建均通过。根目录 exe 已更新，SHA256 为 `de854850867efb009d176d032916f7ddfcc9c62798fc25e15208419bfb5b6034`。
- 当前没有待批准的翅膀候选；下一步优先原生 Windows 桌面验收，再按实际效果决定是否增加收手／归位动作。
- 完整本阶段总结与交付见 [工作总结](docs/work-summary-2026-09-11.md)；运行演示与备份说明见 [正式接入记录](docs/flipper-hold-integration-2026-09-11/README.md)。
- 生图或 API 失败不得修改工作网络配置；第三方收费服务须已有明确授权。新动作初版仍先让用户审核。

> 以下章节保留早期项目设计与历史工作流。涉及“当前任务”、旧帧数、旧文件路径和旧几何尺寸时，以本节、最新工作总结及 `src/config/*.json` 为准；不要恢复旧待办或覆盖已批准的正式素材。
